...
class RBViewAndEditToDoPage extends StatefulWidget {
  const RBViewAndEditToDoPage(
      {super.key, required this.dataofthetask, required this.docid});
  final Map<String, dynamic> dataofthetask;
  final String docid;
  @override
  State<RBViewAndEditToDoPage> createState() => _RBViewAndEditToDoPageState();
}

class _RBViewAndEditToDoPageState extends State<RBViewAndEditToDoPage> {
...
// Restore task from recyclebin
  void restoretask() {
    // create/copy the task to recyclebin collection
    fb.collection("ToDo").doc(widget.docid).set({
      ...
      fb.collection("Recycle_Bin").doc(widget.docid).delete().then((value) {
        tostme("Task Restored.....", 500);
        Navigator.pop(context);
      });
    });
  }

//delete todo data from firestore
  void deletePermanently() {
    fb.collection("Recycle_Bin").doc(widget.docid).delete().then((value) {
      tostme("Task Deleted.....", 500);
      Navigator.pop(context);
    });
  }
